...
- `lib/editais.ts` — funções puras do front (limpeza de título, caixa, agrupamento por prazo, urgência, filtros, facetas, ordenação, janela de inscrição). Mesma disciplina do scraper: sem React, testáveis offline em `tests/editais.test.ts`.
- `app/` + `componentes/` — grade estática (SSG) de cartões com barra lateral de filtros facetados, tudo client-side. `BarraLateral` (órgão / área / faixa de prazo, com contadores), `CartaoEdital` (um edital) e `Radar` (casca e estado).
- Os contadores da barra são facetados: cada dimensão é contada ignorando o próprio filtro, senão as opções não selecionadas mostrariam zero e a barra viraria beco sem saída.
- Busca, órgãos, áreas, faixa de prazo e ordenação viram query string compartilhável (com debounce); a área escolhida também persiste em `localStorage` (`lib/preferencias.ts`). `fonte=finep` isolado continua válido — links antigos não quebram.
- A barra da janela de inscrição só aparece quando a fonte publica início E fim. Metade do dataset não publica início, e barra cheia sem essa ponta seria invenção.
